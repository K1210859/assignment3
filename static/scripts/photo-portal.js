...
function make_photos_call() {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            for(i=0;i<photo_images.length; i++) {
              ...
              photo_anchor_id = photo_name + "-photo-li";
              ...
              if (document.getElementById(photo_anchor_id) == null) {
                ...
                // Check if this is admin user (has radio buttons)
                var isAdmin = document.querySelector('input[name="photo_selection"]') !== null;
                
                if (isAdmin) {
                  // Admin user - add radio button
                  var radio = document.createElement("input");
                  radio.type = "radio";
                  radio.name = "photo_selection";
                  radio.value = photo_name;
                  radio.id = photo_name + "-radio";
                  photo_anchor_li.appendChild(radio);
                  
                  var label = document.createElement("label");
                  label.setAttribute("for", photo_name + "-radio");
                  photo_anchor_li.appendChild(label);
                }
                ...
                if (isAdmin) {
                  photo_anchor_li.querySelector("label").appendChild(photo_anchor);
                } else {
          ...
          var thumbnailList = document.getElementById("available_photos_list");
          var thumbnailChildren = thumbnailList.children;
          for (var i = thumbnailChildren.length - 1; i >= 0; i--) {
            var child = thumbnailChildren[i];
            if (present_thumbnail_photo_list.indexOf(child.id) === -1) {
              thumbnailList.removeChild(child);
            }
          }
          ...
          var photoChildren = photoList.children;
          for (var i = photoChildren.length - 1; i >= 0; i--) {
            var child = photoChildren[i];
            if (present_photo_anchor_list.indexOf(child.id) === -1) {
              photoList.removeChild(child);
            }
          }
...
function start_slide_show() {
  ...
  xhr.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      var photo_images = response.photos;
      
      if (photo_images.length > 0) {
        display_photo(0, photo_images);
      } else {
        alert('No photos available for slideshow.');
      }
    }
  };
...
function handle_update_delete_form(action) {
    var selectedPhoto = document.querySelector('input[name="photo_selection"]:checked');
    if (selectedPhoto) {
        if (action === "delete") {
            // Create a hidden input for photo name
            var hiddenInput = document.createElement('input');
            hiddenInput.type = 'hidden';
            hiddenInput.name = 'photo_name';
            hiddenInput.value = selectedPhoto.value;
            
            // Add to form and submit
            var form = document.getElementById('update_delete_form');
...
// Function to show update tags modal
function update_tags_modal() {
    var selectedPhoto = document.querySelector('input[name="photo_selection"]:checked');
    if (selectedPhoto) {
        var photoName = selectedPhoto.value;
        
        // Make AJAX call to get current tags
        var xhr = new XMLHttpRequest();
        xhr.open('GET', '/photos/' + encodeURIComponent(photoName) + '/tags', true);
        
        xhr.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                var response = JSON.parse(this.responseText);
                var currentTags = response.tags || '';
                
                // Populate modal
                document.getElementById('currentPhotoName').textContent = photoName;
                document.getElementById('currentTags').textContent = currentTags;
                document.getElementById('newTags').value = currentTags;
                document.getElementById('hiddenPhotoName').value = photoName;
...
function delete_photo() {
    var selectedPhoto = document.querySelector('input[name="photo_selection"]:checked');
    if (selectedPhoto) {
        if (confirm('Are you sure you want to delete ' + selectedPhoto.value + '?')) {
            handle_update_delete_form("delete");
        }
    } else {
        alert('Please select a photo first.');
    }
