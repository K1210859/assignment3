...
def get_tags_for_photo(photo):
    ...
    session = DBSession()
    photo_data = session.query(PhotoData).filter(PhotoData.name == photo).first()
    
    if photo_data:
        tags_to_return = photo_data.tags
    else:
        tags_to_return = ""
...
def update_tags():
    ...
    session = DBSession()
    photo_data = session.query(PhotoData).filter(PhotoData.name == photo_name).first()
    
    if photo_data:
        photo_data.tags = new_tags
        session.commit()
        status = f"Tags successfully updated for:{photo_name}"
    else:
        status = f"Photo {photo_name} not found"
...
def delete_photos():
    ...
    if photo_data:
        session.delete(photo_data)
        session.commit()
        status = f"Photo with name {photo_name} deleted."
        
        # Also delete the physical file
        import os
        file_path = os.path.join(PHOTO_FOLDER, photo_name)
...
def upload_photo():
	...
	session = DBSession()
	existing_photo = session.query(PhotoData).filter(PhotoData.name == filename).first()
	
	if existing_photo:
		# Photo with same name already exists
		status = f"Photo {filename} already exists."
		session.close()
	else:
		# Photo doesn't exist, save it
		photo_data = PhotoData(name=filename, date_taken=date_taken, tags=tags)
		session.add(photo_data)
		session.commit()
		session.close()
		status = f"Photo {filename} uploaded successfully."
